...
public class KitchenTicketView extends JPanel {
	...
	private void createHeader(KitchenTicket ticket) {
		...
		ticketInfo = new JLabel(POSConstants.TIQUETE_KITCHEN + ticket.getTicketId() + "-" + ticket.getSequenceNumber() + " " + printerName + ""); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
		...
		if (ticket.getTableNumbers() != null && ticket.getTableNumbers().size() > 0) {
			...
			tableInfo.setText(POSConstants.TABLE_KITCHEN + tableNumbers); //$NON-NLS-1$
		}
		serverInfo = new JLabel();
		if (ticket.getServerName() != null) {
			serverInfo.setText(POSConstants.SERVER_KITCHEN + ticket.getServerName()); //$NON-NLS-1$
		}
