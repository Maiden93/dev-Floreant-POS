...
public class OrderView extends ViewPanel {
	...
	private com.floreantpos.swing.PosButton btnSeatNo = new com.floreantpos.swing.PosButton("ASIENTO:");
	...
	private PosButton btnDeliveryInfo = new PosButton("INFORMACIÓN DE ENTREGA");
	...
	private void addActionButtonPanel() {
		...
		btnCancel.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent evt) {
				...
				int result = POSMessageDialog.showYesNoQuestionDialog(null, POSConstants.SEND_ORDER_KITCHEN, "Confirmar");
		...
		btnSend.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					POSMessageDialog.showMessage(POSConstants.ENVIAR_ORDEN_COCINA);
				} catch (StaleObjectStateException x) {
					POSMessageDialog.showError(Application.getPosWindow(), Messages.getString("TicketView.22")); //$NON-NLS-1$
					return;
				} catch (PosException x) {
		...
		btnHold.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (orderType.isShowTableSelection() && orderType.isRequiredCustomerData()//fix
					...
					String password = PasswordEntryDialog.show(Application.getPosWindow(), POSConstants.INGRESAR_PASS);
					...
					if (user2 == null) {
						POSMessageDialog.showError(Application.getPosWindow(), POSConstants.ERROR_PASS);
						return;
					}
					else {
	...
	protected void doAddSeatNumber() {// GEN-FIRST:event_btnCustomerNumberActionPerformed
		...
		seatDialog.setTitle("Seleccione Asiento");
		seatDialog.pack();
		seatDialog.open();
		...
		if (seatNumber == -1) {
			NumberSelectionDialog2 dialog = new NumberSelectionDialog2();
			dialog.setTitle("Introduzca el número de asiento");
			dialog.pack();
			dialog.open();
		...
		if (currentTicket != null) {
			...
			if (!type.isShowTableSelection()) {
				...
				if (tableNumbers != null) {
					...
					btnTableNumber.setText("<html><center>" + "MESA" + ": " + tables + "</center><html/>");

				}
				else {
					btnTableNumber.setText("MESA");
				}

				btnGuestNo.setText("INVITADOS" + ": " + String.valueOf(currentTicket.getNumberOfGuests()));
			}
