	...
	public void itemSelected(MenuItem menuItem) {
		...
		if (menuItem.isFractionalUnit()) {
			if (TerminalConfig.getScaleActivationValue().equals("cas10")) {
				...
				itemQuantity = BasicWeightInputDialog.takeDoubleInput("Introduzca el peso o la cantidad del artículo.", 1);
			}
			if (itemQuantity <= -1) {
				return;
			}
			...
			if (itemQuantity == 0) {
				POSMessageDialog.showError("La unidad no puede ser cero");
				return;
			}
		}
		...
		if (!orderView.getTicketView().isStockAvailable(menuItem, ticketItem, -1)) {
			POSMessageDialog.showError("Los artículos no están disponibles en stock");
