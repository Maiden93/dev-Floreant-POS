...
public class LanguageSelectionDialog extends POSDialog {
	...
	private void init() {
		...
		titlePanel.setTitle(Messages.getString("LanguageMenu.2")); //$NON-NLS-1$
		...
		JLabel lblLanguage = new JLabel(Messages.getString("LanguageMenu.3"));
		...
		PosButton btnSave = new PosButton(Messages.getString("LanguageButtonSave"));
		...
		PosButton btnCancel = new PosButton(Messages.getString("LanguageButtonCancel"));
	...
	public void doSave() {
		...
		if (cbLang.getSelectedItem() != null) {
			...
			POSMessageDialog.showMessage(this, Messages.getString("LanguageMenu.0"));
			dispose();
			try {
				Main.restart();
			} catch (Exception e) {
