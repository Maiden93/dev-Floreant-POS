...
public class OrderFilterPanel extends JXCollapsiblePane {
	...
	private void createPaymentStatusFilterPanel() {
		btnFilterByOpenStatus = new POSToggleButton(PaymentStatusFilter.ABIERTOS.toString());
		btnFilterByPaidStatus = new POSToggleButton(PaymentStatusFilter.PAGADOS.toString());
		btnFilterByUnPaidStatus = new POSToggleButton(PaymentStatusFilter.CERRADOS.toString());
		...
		ActionListener psFilterHandler = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (actionCommand.equals("CERRADOS") && !Application.getCurrentUser().hasPermission(UserPermission.VIEW_ALL_CLOSE_TICKETS)) {

					String password = PasswordEntryDialog.show(Application.getPosWindow(), "Ingresa una contraseña privilegiada");
					...
					if (user2 == null) {
						POSMessageDialog.showError(Application.getPosWindow(), "Ningún usuario encontrado con esa contraseña");
						updateButton();
						return;
					}
					else {
						if (!user2.hasPermission(UserPermission.VIEW_ALL_CLOSE_TICKETS)) {
							POSMessageDialog.showError(Application.getPosWindow(), "Sin permisos");
							updateButton();
							return;
						}
					}
		...
		/*btnFilterByOpenStatus.setText("OPEN");
		btnFilterByPaidStatus.setText("PAG");
		btnFilterByUnPaidStatus.setText("CERR");*/
	...
	private void updateButton() {
		PaymentStatusFilter paymentStatusFilter = TerminalConfig.getPaymentStatusFilter();
		if (paymentStatusFilter.name().equals("ABIERTOS")) {
			btnFilterByOpenStatus.setSelected(true);
			
		}
		else if (paymentStatusFilter.name().equals("PAGADOS")) {
			btnFilterByPaidStatus.setSelected(true);
			//btnFilterByPaidStatus.setText("PAGADAS");
		}
		else if (paymentStatusFilter.name().equals("CERRADOS")) {
			btnFilterByUnPaidStatus.setSelected(true);
			//btnFilterByUnPaidStatus.setText("CERRADAS");
		}
	}
